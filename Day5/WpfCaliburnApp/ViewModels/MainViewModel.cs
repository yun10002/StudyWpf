...
namespace WpfCaliburnApp.ViewModels
{
    ...
    {
        //private EmployeesModel employee; //VS 제안 이름 사용 권장
        ...
        string connString = "Data Source=DESKTOP-TSOF850;Initial Catalog=OpenApiLab;Integrated Security=True";
        ...
        //버튼 활성/비활성화를 속성
        public bool CanSaveEmployee //밑의 SaveEmployee와 이름 같아야하고 앞에 Can 붙인 거!
        {
            get {
        ...
        public void SaveEmployee()
        {
            using (SqlConnection conn=new SqlConnection(connString))
            {
                ...
                if (Id != 0) //update일 때는 Id 값도 파라미터로 추가해야함
                {
            ...
            NewEmployee();  //입력 창 전부 초기화
            GetEmployees(); //데이터 다시 조회(데이터그리드 호출)
        }

        public bool CanDelEmployee
        {
            get { return (Id != 0); } //Id가 0이 아니면 삭제 가능
        }
